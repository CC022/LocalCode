...
- **`Views/`** — SwiftUI views that read `AppState`.

### The agent loop (the thing that matters)

`AgentLoop.send(_:)` is a `while true` that mirrors `learn-claude-code-main/s02_tool_use/code.py`:
...
1. New file under `Agent/Tools/`, struct conforming to `Tool` protocol (in `Agent/Tool.swift`).
...
The Python source of truth lives in `learn-claude-code-main/` (e.g. `s02_tool_use/code.py` is what `AgentLoop.swift` ports). When porting the next chapter, read both the chapter's `code.py` and its `README*.md` — the README explains the *why* behind the mechanism the chapter introduces.
